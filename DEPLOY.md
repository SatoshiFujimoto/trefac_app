...
マスター入力CSVを Spaces の `trefac/` フォルダにアップロード（DOコンソールのドラッグ&ドロップでOK）:

- ファイル名は自由（例 `aki - sushionsen_up_04 (17).csv`、将来 `aki - sushionsen_up_004 (1).csv` でも可）。
- `fetch_input.py` は **`trefac/` 配下で最新更新の .csv** を取得する。
- 運用上は **常に1個だけ**置く（古いものは消す）。複数あっても最新が選ばれるので事故にはならない。

更新時はこのフォルダに新版を上げる（＋古いものを消す）だけで全箱に反映される。
※箱用キーは Read-only。Spaces配下の一覧取得(List)も Read 権限に含まれる必要がある。
...
#                       prefix=trefac/ / local_file=aki - sushionsen_up_04_master.csv
...
# (4) key/ のOAuth jsonは trefac本体では未使用 → コピー不要
#     （eBayトークンは[Credentials]のrefresh_token、メールはgmail_app_passのSMTPで完結）
...
- 入力リストの変更 → Spaces の `trefac/` に新版CSVを上げる（古いものは消す）だけ。
